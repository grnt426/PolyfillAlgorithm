			...
			for (int edge = 0; edge < aet.size(); edge++) {
				...
				if (aet.get(edge)[EDGE_DY] != 0) {
					...
					if (aet.get(edge)[EDGE_C] < 0)
						aet.get(edge)[EDGE_C] = (aet.get(edge)[EDGE_C]
								% aet.get(edge)[EDGE_DY]) * -1;
					else
						aet.get(edge)[EDGE_C] = aet.get(edge)[EDGE_C]
								% aet.get(edge)[EDGE_DY];
				}
	...
	private void buildGlobalEdgeTable(int n, int[] x, int[] y) {
		...
		for (int vertex = 0; vertex < n; vertex++) {
			...
			if (y[vertex] >= y[next_vertex]) {
				vertex_data[0] = y[vertex];
				vertex_data[1] = x[next_vertex];
			} else {
				vertex_data[0] = y[next_vertex];
				vertex_data[1] = x[vertex];
			}

			// Store the slope of the edge
			vertex_data[2] = x[vertex] - x[next_vertex];
			vertex_data[3] = y[vertex] - y[next_vertex];
			vertex_data[4] = 0;
